...
<body>
  <div class="container-lg">
    ...
    <div class="botones">
      <button type="button" type="button" class="btn agregar" data-bs-toggle="modal" id="btn_Guardar" data-bs-target="#departamentoModal_agregar" onclick="seleccionaDepartamento(<?php echo 1 . ',' . 1 ?>);">Agregar</button>
        ...
        <tbody style="font-family:Arial;font-size:12px;text-align: center;">
          <?php foreach ($departamentos as $dato) { ?>
            <tr>
              ...
              <td style="width: 70px;"><button type="button" class="btn btn-light" data-bs-toggle="modal" data-bs-target="#departamentoModal_agregar" onclick="seleccionaDepartamento(<?php echo $dato['id'] . ',' . 2 ?>);"><i class="bi bi-pencil-square"></i></button></td>
              <td style="width: 70px;"><button type="button" class="btn btn-light"  data-bs-toggle="modal" data-bs-target="#modal-confirma" data-href="<?php echo base_url('/departamentos/eliminar') . '/' .$dato['id']. '/' .'E'; ?>"><i class="bi bi-trash3"></i></button></td>
            </tr>
          <?php } ?>
        </tbody>
      </table>
    ...
    <form method="POST" action="<?php echo base_url('/departamentos/insertar'); ?>" autocomplete="off">
      <div class="modal fade" id="departamentoModal_agregar" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
          <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h1 class="modal-title fs-5" id="titulo">Agregar Departamentos</h1>
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
              <!-- <label for="exampleDataList" class="form-label">Nombre del Pais</label> -->
              <select class="form-select" name="pais" id="pais">
                <option selected>Selecciona un pais</option>
                <?php foreach ($paises as $dato) { ?>
                  <option value="<?php echo $dato['id']; ?>" name="pais"><?php echo $dato['nombre']; ?></option>
                <?php } ?>
              </select>
              <label for="exampleDataList" class="form-label">Nombre</label>
              <input class="form-control" list="datalistOptions" id="nombre" name="nombre">
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div  class="modal-content">
                <div style="text-align:center;" class="modal-header">
                    <h5 style="font-size:20px;font-weight:bold;" class="modal-title" id="exampleModalLabel">Eliminación de Registro</h5>
  ...
  function seleccionaDepartamento(id, tp) {
    ...
    if (tp == 2) {
      ...
      $.ajax({
        ...
        dataType: "json",
        success: function(rs) {
          document.getElementById('exampleModalLabel').innerText = "Editar Departamento";  
          $("#tp").val(2);  
          $("#id").val(id);  
          $("#pais").val(rs[0]['id_pais']);
          $("#nombre").val(rs[0]['nombre']);
          $("#btn_Guardar").text('Actualizar');
          $("#departamentoModal_agregar").modal("show");
        }
      })
    } else {
