...
<div class="container-lg">
    <h1 class="titulo" style="color: #93623c;text-align: center;">Municipios Eliminados</h1>
    <div class="botones align-baseline" style=" margin: 30px 6px;height: 60px;">
        <a style="border-color: none;color: white;background-color: #93623c;" href="<?php echo base_url('/municipios'); ?>" class="btn  regresar_Btn">Regresar</a>
    </div>
          ...
          <tbody style="font-family:Arial;font-size:12px;text-align: center;">
            <?php foreach ($municipios as $dato) { ?>
              <tr>
                ...
                <td style="width: 70px;"><button type="button" class="btn btn-light" data-href="<?php echo base_url('/municipios/eliminar') . '/' .$dato['id']. '/' .'A'; ?>" data-bs-toggle="modal" data-bs-target="#modal-confirma"><i class="bi bi-arrow-repeat"></i></button></td>
              </tr>
            <?php } ?>
          </tbody>
        </table>
...
<script>
  ...
  function eliminarMunicipio(id) {     
      $("#id").val(id);
      dataURL = "<?php echo base_url('eliminar_municipio'); ?>" + "/" + id + "/" + 'A';
      $.ajax({
        type: "POST",
        url: dataURL,
        dataType: "json",
